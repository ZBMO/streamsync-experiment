<template>
	<div class="CoreTextInput" ref="rootEl" onclick="alert('text input clicked')">
		<label>{{ fields.label.value }}</label>
		<input
			:type="fields.passwordMode.value == 'yes' ? 'password' : 'text'"
			:value="formValue"
			v-on:input="($event) => customHandler(($event.target as HTMLInputElement).value, 'ss-change')"
			v-on:change="($event) => customHandler(($event.target as HTMLInputElement).value, 'ss-change-finish')"
...
function customHandler(event, type) {
	

		const customPayload = {
			value: event,
			componentId: fields.label.value ,
		}

		console.log("customHandler payload: ")
		console.log(customPayload)

		handleInput(customPayload, type)
}
